...
const WAMessageHandler = require('./advanced-message-handler');
...
const makeMessagesSocket = (config) => {
    ...
    const messageHandler = new WAMessageHandler(Utils_1, waUploadToServer, relayMessage);
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            const { filter = false, quoted, ephemeralExpiration: _unused, ...restOptions } = options;
            const getParticipantAttr = () => filter ? { participant: { jid } } : {};
            const messageType        = messageHandler.detectType(content);

            // ── Handle disappearing messages toggle ───────────────────────────
            if (
            ...
            if (messageType) {
                switch (messageType) {
                    case 'PAYMENT': {
                        const paymentContent = await messageHandler.handlePayment(content, quoted);
                    ...
                    case 'PRODUCT': {
                        const productContent = await messageHandler.handleProduct(content, jid, quoted);
                        const productMsg     = await Utils_1.generateWAMessageFromContent(jid, productContent, { quoted });
                        return relayMessage(jid, productMsg.message, {
                            messageId: productMsg.key.id,
                            ...getParticipantAttr(),
                        });
                    }
                    case 'INTERACTIVE': {
                        const interactiveContent = await messageHandler.handleInteractive(content, jid, quoted);
                        const interactiveMsg     = await Utils_1.generateWAMessageFromContent(jid, interactiveContent, { quoted });
                        return relayMessage(jid, interactiveMsg.message, {
                            messageId: interactiveMsg.key.id,
                            ...getParticipantAttr(),
                        });
                    }
                    case 'ALBUM':
                        return messageHandler.handleAlbum(content, jid, quoted);
                    case 'EVENT':
                        return messageHandler.handleEvent(content, jid, quoted);
                    case 'POLL_RESULT':
                        return messageHandler.handlePollResult(content, jid, quoted);
                    case 'GROUP_STORY':
                        return messageHandler.handleGroupStory(content, jid, quoted);
                    default:
                        break;
                }
            }
            ...
            const fullMsg = await Utils_1.generateWAMessage(jid, content, {
                ...
                upload: async (readStream, opts) => {
                    return waUploadToServer(readStream, {
                        ...opts,
